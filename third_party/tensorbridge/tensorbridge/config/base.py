...
def name_value_to_macro_style(name: str, value: Any) -> str:
    ...
    if isinstance(value, (bool, int)):
        value = int(value)
        return f"#define TENSORBRIDGE_{name.upper()} {int(value)}"
    return ""


@dataclasses.dataclass
class BaseTensorBridgeConfig:
    ...
    def to_cpp_str(
        self,
        cls: type["BaseTensorBridgeConfig"] | None = None,
    ...
    def to_macro_cpp_str(self, cls: type["BaseTensorBridgeConfig"] | None = None) -> str:
        cls = cls or self.__class__
        str_list = []
        names = [x.name for x in dataclasses.fields(cls)]
        names += list(cls._cpp_extra_names)
    ...
    def to_extern_cpp_str(self, cls: type["BaseTensorBridgeConfig"] | None = None) -> str:
        cls = cls or self.__class__
        str_list = []
        names = [x.name for x in dataclasses.fields(cls)]
        names += list(cls._cpp_extra_names)
