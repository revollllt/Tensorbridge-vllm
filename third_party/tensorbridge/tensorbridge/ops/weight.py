...
from tensorbridge.kernel.process_mxfp4 import ProcessMxfp4W4A8Kernel
...
def process_mxfp4_w4a8_weight(
    ...
    # Kernel processes one uint2 (= 2 int32 = 16 fp4 nibbles = one group_size=16
    # group) per thread. For larger group sizes the offset is broadcast across
    # the multiple uint2 chunks it covers.
    assert inputs.nelement() >= delta_scale_offsets.nelement() * 2

    def is_power_of_two(n: int) -> bool:
        return n > 0 and (n & (n - 1)) == 0

    repeat_count = inputs.nelement() // 2 // delta_scale_offsets.nelement()
