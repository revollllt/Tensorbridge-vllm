...
from tensorbridge.schema.base import BaseWeightSchema
from tensorbridge.schema.tensorbridge import TensorBridgeWeightSchema
...
class BitnetWeightSchema(BaseWeightSchema):
    ...
    def process_loaded_weight(self, tensor: torch.Tensor, name: str) -> torch.Tensor:
        if name == "weight":
            ...
            tensor = _ops().unpack_weight(tensor, 2)
            tensor = tensor.view(e, tensor.size(-2), -1, 4)
            tensor = tensor.transpose(-1, -2).contiguous()
            tensor = tensor.view(e, tensor.size(-3), -1)
            tensor = _ops().pack_weight(tensor, 2).view(torch.uint8)
            tensor = tensor.transpose(-1, -2).contiguous()
    ...
    def convert_tensorbridge(
        ...
        weight = _ops().unpack_weight(weight, 2) + 1
        weight = weight.transpose(-1, -2).contiguous()
        weight = _ops().pack_weight(weight, 2)
