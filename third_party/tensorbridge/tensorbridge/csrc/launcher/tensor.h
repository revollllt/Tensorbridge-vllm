...
inline void check_tensor_b(Tensor &tensor, KernelData &kernel_data, int64_t dev) {
  ...
  if (kernel_data.use_tma_b_swizzle_64) {
    // swizzle64_raw layout: plain (N, K_int4 * b_bits / 32) row-major int32.
...
inline CUtensorMap make_tma_desc_b(Tensor &tensor, KernelData &kernel_data) {
  ...
  if (kernel_data.use_tma_b_swizzle_64) {
    // CUTLASS-aligned 2D + 64B-swizzle TMA-B descriptor.
    // CUTLASS uses Layout<<C<8>, C<128>>, ...> (8 rows × 128 int4 per row, row-major,
    // 64B per row, ComposedLayout with Swizzle<2,4,3>=64B).
    // Tensor is stored as int32 with 8 int4 packed per int32:
    //   element_size = 4 bytes, last dim = K_int4 / 8 = K/8 int32 elements.
    // SMEM box per stage: BlockShape::N rows × BlockShape::K_int4 cols
    //                   = block_shape_n × (block_shape_k * num_bits / 32) int32
    //                   = block_shape_n × (block_shape_k / 8) int32
    // For our shape: 128 × 16 int32 = 128 rows × 64 bytes = 8192 bytes per stage.
    // Inner dim must equal 64-byte swizzle row exactly (i.e., 16 int32).
    tensor = torch_view_shape(tensor, {-1, tensor.size(-1)});
    return make_tma_desc(
        tensor,
        {block_shape_k * num_bits / 32, block_shape_n},  // {K-inner int32, N rows}
        64);
  }
...
inline CUtensorMap make_tma_desc_c(Tensor tensor, KernelData &kernel_data) {
  ...
  tensor = torch_view_shape(tensor, {-1, tensor.size(-1)});
  // Both the scalar baseline and the STSM (Strategy-B) epilogue stage a 128B-swizzled
  // [token][channel] tile (Swizzle<3,4,3>); the STSM store applies the matching XOR
  // (chan_grp ^= token%8) so the descriptor round-trips bit-exact to GMEM[token][channel].
